...
async function apiRequest<T>(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`API Error [${response.status}]: ${fetchOptions.method || "GET"} ${url}`, errorMessage);
    ...
    // Network errors
    console.error(`Network Error: ${fetchOptions.method || "GET"} ${url}`, error);
    throw new ApiClientError(
      0,
      error instanceof Error ? error.message : `Network error occurred. Please ensure the backend is running at ${API_BASE_URL}`,
    );
  }
}
