...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  try {
    ...
    stats = await api.get(`/organizations/${user.organizationId}/stats`, {
      token: token || undefined,
    });
  } catch {
    // Ignore errors
...
export default function Dashboard() {
  const { user, events, stats, subscription, organizationStats } = useLoaderData<typeof loader>();
  const fetcher = useFetcher();

  const subscriptionPlan = subscription?.planName || "premium";
  const eventCount = organizationStats?.events?.total || stats.totalEvents;
  const isPremium = subscriptionPlan === "premium";
  const hasReachedLimit = isPremium && eventCount >= 1;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-gray-600 mt-1">
                {isPremium ? (
                  <>
                    Events Created: {eventCount} / 1 free event
                    {!hasReachedLimit && (
                      <span className="ml-2 text-green-600">({1 - eventCount} remaining)</span>
                    )}
                  </>
                ) : (
                  <>Unlimited events • {eventCount} created</>
                )}
              </div>
            </div>
            {hasReachedLimit && (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 p-4 sm:p-6 rounded-xl shadow-lg text-white">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs sm:text-sm opacity-90 mb-1">Planning</div>
              <div className="text-2xl sm:text-3xl font-bold">{stats.planningEvents}</div>
            </div>
            <svg className="w-8 h-8 sm:w-10 sm:h-10 opacity-75" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-gray-100">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          ...
          {(user.role.toLowerCase() === "admin" || user.role.toLowerCase() === "eventmanager" || user.role.toLowerCase() === "finance") && (
            <Link
              to="/expenses"
            ...
            >
              ...
              <span className="text-sm font-medium text-purple-900">Expenses</span>
            </Link>
          )}
        </div>
      </div>
