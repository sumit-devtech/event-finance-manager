import { ArrowRight, CheckCircle, BarChart3, Users, DollarSign, Zap, Shield, Globe } from './Icons';
...
export default function LandingPage({ onDemoMode, onGetStarted }: LandingPageProps) {
  const features = [
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 md:py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <img src={owlLogo} alt="Simplifi Logo" className="w-8 h-8 md:w-10 md:h-10" />
              <h1 className="text-blue-600 text-lg md:text-2xl">Simplifi</h1>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
        <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-center">
          <div className="text-center lg:text-left">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl mb-4 md:mb-6 bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent leading-tight">
              Ready to <span className="text-orange-500">Simplifi</span> your Event Budgeting?
            </h2>
            <p className="text-base md:text-lg lg:text-xl text-gray-600 mb-6 md:mb-8">
              Built for the teams behind great events. Plan, track, and optimize your event budgets with powerful analytics and team collaboration tools.
            </p>
          ...
          <div className="flex justify-center">
            <img src={owlLogo} alt="Simplifi - Your Event Budget OS" className="w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96 object-contain" />
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
        <div className="text-center mb-8 md:mb-12">
          <h3 className="text-2xl md:text-3xl mb-2 md:mb-4">Everything you need to manage events</h3>
            ...
            return (
              <div key={index} className="bg-white p-4 md:p-6 rounded-xl border border-gray-200 hover:shadow-lg transition-shadow">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-blue-100 rounded-lg flex items-center justify-center mb-3 md:mb-4">
                  <Icon size={20} className="text-blue-600 md:w-6 md:h-6" />
                </div>
                <h4 className="mb-2 text-base md:text-lg">{feature.title}</h4>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
        <div className="text-center mb-8 md:mb-12">
          <h3 className="text-2xl md:text-3xl mb-2 md:mb-4">Simple, transparent pricing</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
          {plans.map((plan, index) => (
            ...
            >
              ...
              <h4 className="mb-2 text-lg md:text-xl">{plan.name}</h4>
              <div className="mb-2">
                <span className="text-3xl md:text-4xl">{plan.price}</span>
                {plan.price !== 'Custom' && <span className="text-gray-600 text-sm md:text-base">/month</span>}
              </div>
              <p className="text-gray-600 text-sm md:text-base mb-4 md:mb-6">{plan.description}</p>
              <button
                onClick={onGetStarted}
                className={`w-full py-2.5 md:py-3 rounded-lg mb-4 md:mb-6 transition-colors text-sm md:text-base ${
                  plan.highlighted
              ...
              <ul className="space-y-2 md:space-y-3">
                {plan.features.map((feature, fIndex) => (
                  <li key={fIndex} className="flex items-start gap-2">
                    <CheckCircle size={18} className="text-green-600 flex-shrink-0 mt-0.5 md:w-5 md:h-5" />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20">
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl md:rounded-2xl p-6 md:p-12 text-center text-white">
          <h3 className="text-2xl md:text-3xl mb-3 md:mb-4">Ready to get started?</h3>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4">
            <button
              onClick={onGetStarted}
              className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 bg-white text-blue-600 rounded-lg hover:bg-gray-100 transition-colors text-sm md:text-base"
            >
              Start Free Trial
            </button>
            <button
              onClick={onDemoMode}
              className="w-full sm:w-auto px-6 md:px-8 py-3 md:py-4 border-2 border-white text-white rounded-lg hover:bg-white/10 transition-colors text-sm md:text-base"
            >
              Try Demo
            </button>
          </div>
